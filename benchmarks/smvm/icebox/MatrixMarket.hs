...
--
data Matrix
  = PatternMatrix !(Int,Int) !Int !Structure [(Int,Int)]
  | IntMatrix     !(Int,Int) !Int !Structure [(Int,Int,Int)]
  | RealMatrix    !(Int,Int) !Int !Structure [(Int,Int,Double)]
  | ComplexMatrix !(Int,Int) !Int !Structure [(Int,Int,Complex Double)]
  deriving Show
...
matrix = do
  ...
  case t of
    Real    -> RealMatrix    (m,n) l s <$> many1' (line floating)
    Complex -> ComplexMatrix (m,n) l s <$> many1' (line ((:+) <$> floating <*> floating))
    Integer -> IntMatrix     (m,n) l s <$> many1' (line integral)
    Pattern -> PatternMatrix (m,n) l s <$> many1' ((,) <$> integral <*> integral)
