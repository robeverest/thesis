...
THREADS=1
SMVM=$(stack exec which -- smvm-mkl)
...
for matrixFile in ${MATRICES[@]}; do
  ...
  du -h $matrixFile
  env MKL_NUM_THREADS=$THREADS numa_spreadout $THREADS $SMVM $matrixFile --output=$outputBase.html --raw=$outputBase.raw 2>&1 | tee $outputBase.log
