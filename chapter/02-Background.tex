...
The Nested Data Parallel (NDP) programming model is the most general form of data parallelism. It allows for the arbitrary nesting of parallel operations within other parallel operations, as well as parallel arrays within other parallel arrays. It is an expressive, high level and modular method of parallel programming~\cite{Blelloch:1990vl}. A variety of different useful algorithms have been implemented in languages supporting this model~\cite{Blelloch:nbody94,Blelloch:delaunay96,Blelloch:connected94}.

Nested data parallelism (NDP) was first popularised by the \nesl{} language~\cite{Blelloch:nesl1995}, but it currently exists in many different languages, albeit in a restricted form in some. These include Proteus~\cite{proteus-frontiers95}, Futhark~\cite{Henriksen:2017futhark}, Nova~\cite{collins2013nova}, Manticore~\cite{Fluet:2007:manticore}, Lift~\cite{Steuwer:lift}, Copperhead~\cite{Catanzaro:copperhead}, Nikola~\cite{Mainland:nikola}, and Data Parallel Haskell (DPH)~\cite{Jones:2008uu}. These languages implement nested parallelism by a variety of means. Manticore has an execution model based on fork-join parallelism. Lift, Copperhead, and Nikola directly map nested parallel combinators to the parallel hierarchy of GPUs. The rest rely on the program transformation we introduced in Chapter~\ref{sec:introduction}. First described by \citet{Blelloch:compiling1988}, the flattening transform flattens nested parallel programs while preserving behaviour. This technique is the one we use in our work, so will describe it here.
...
Blelloch's Transform has two distinct components. The first being a \emph{flattened array representation}, the second, program flattening. We will address each of these in turn.

\subsection{Array representation}
\label{sec:background-representation}

In practice, computing with nested arrays requires a means for representing nested structure in the flat memory provided by the von Neumann model we are used to.

This is an example of a simple nested array:
%
\begin{lstlisting}
{ {A,B,C}, {D,E}, {F,G,H,I}, {}, {J} }
\end{lstlisting}
%
Let us assume that @A@ to @J@ are scalar elements. That gives this array one level of nesting. The immediately obvious way of representing this is with an array of pointers. Indeed, most contemporary general-purpose languages represent nested arrays in this way. Note that this is distinct from multidimensional arrays. While one can treat, say, a 2D array as an array of arrays, the fact that each \emph{subarray} has to be the same length does not make it a true nested array structure.

So if an array of pointers allows us to represent arbitrary nested structures, what is the problem? It lies in the fact that modern computer hardware is optimised for locality of reference. Having different (often small) parts of the structure spread over memory with pointers between them means that any operations performed over them have to continually chase pointers and access new areas of memory. This results in many cache misses and is unworkable in the context of high performance. In addition, allocating pointer-based structures involves numerous small allocations. This means construction and destruction is expensive, and, in the context of a garbage collector, longer GC pauses.

The alternative solution to this problem, first proposed by \citet{Blelloch:compiling1988}, is \emph{flattening} the array structure, splitting an into an array of \emph{segment descriptors} and an array of values. The example above is represented as:
%
\begin{lstlisting}
...
Further nesting levels can then be represented by adding additional segment descriptors. For example, in this array with a nesting depth of 2:
%
\begin{lstlisting}
{ { {A,B}, {C} },
  { {D,E}, {}, {F} } }
...
There is another form of segment descriptor that solves this problem. Instead of tracking the size of each subarray, it holds the offset. So, for our first example,
%
\begin{lstlisting}
...
It is worth noting that there are other, more complex, nested array representations in existence. In particular \citet{Lippmeier:replicate} describe one that introduces the concept of \emph{virtual segments}. They allow for arrays to be replicated without explicitly storing them more than once. This problem arises when trying to support higher-order nested parallelism: when arrays can store functions as elements. In the context of this work, we do not support higher-order nested parallelism, so we will not explore it in detail.

\subsection{Flattening}

Flattening, sometimes referred to as vectorisation, is a syntax-driven program transformation. When given a program as input it outputs a flat data-parallel program which is semantically equivalent but operates on a flattened array representation. Most, but not all (Proteus~\cite{proteus-frontiers95} being the exception), languages that rely on a flattened array representation also perform a transform like this. In Chapter~\ref{sec:introduction} we first introduced this idea, but before describing it in more detail, we must first discuss \emph{why} it is a necessary in so many cases.
...
\end{lstlisting}
%
How might this program be scheduled on parallel hardware? To start with, lets simply assume that a @map@ operation can either be executed in parallel or sequentially. In the sequential case, the entire input array is processed on one thread, one element at a time, equivalent to implementing the map as a simple @for@ loop. In the parallel case, we assume that at least some of the elements of the array are processed in parallel. There are many possible ways that this could be done, but for the purposes of this explanation we only consider the fact that, for a simple @map@, the parallelism available can be exploited, at least to some degree. The best means by which an array can be divided up amongst multiple processors can depend on the hardware, but it satisfies the explanation in this section to assume that an array of length $n$ is divided up amongst $k$ processors by giving each processor a chunk of $n/k$ elements.

Coming back to the example, there are therefore 4 possible ways it could be executed:

\begin{enumerate}
\item Both maps \emph{sequentially}
\item The inner map sequentially, the outer map in parallel
\item The outer map sequentially, the inner map in parallel
\item Both maps in \emph{parallel}
\end{enumerate}

Of these strategies, (1) doesn't take advantage of any of the available parallelism, so is not tenable in this work; (2) only works well if xs is sufficiently large; and (3) only works if the \emph{elements} of xs are sufficiently large. (4) seems the most obvious choice, as it takes advantage of all the available parallelism. However, it assumes no cost in launching a new parallel operation. In practice, this is rarely the case.

The best possible solution would be if we could treat these two nested maps as one single map. This is what the flattening transform offers us. It converts all nested array operations into operations on flat arrays. This means that as long as we have a means to execute a @map@ in parallel, we also have a means to execute a program with nested @map@s and can take advantage of all available parallelism.

Before looking at the transform itself, we'll start by rewriting @foo@ by hand such that it no longer has the nested @map@s and uses the nested array representation described above.
%
\begin{lstlisting}
...
\end{lstlisting}
%
In this case, we are simply mapping $2x+1$ for all $x$ in the nested arrays. It is trivial to see that @foo@ and @foo'@ are equivalent, just with different structures being employed. As can be seen, @foo'@ gives us exactly what we want as it preserves all available parallelism, but without requiring nested array operations.

Blelloch's flattening transformation is a method for producing these equivalent programs. It works by \emph{lifting} functions into vector space. That is, given some function
%
\begin{lstlisting}
f :: a -> b
\end{lstlisting}
...
\end{lstlisting}
%
removes the intermediate structure from applying @map@ to the result of another @map@. However, this does not work in the presence of let-bindings; for example, something as simple as
%
\begin{lstlisting}
let ys = map g xs in map f ys
\end{lstlisting}
%
may not be fused without additional inlining. Additionally, there is no simple way to remove the intermediate structure in expressions of the form:
%
\begin{lstlisting}
fold f z (map g xs)
\end{lstlisting}
%
At first glance, this may appear untrue, as
%
\begin{lstlisting}
fold f z (map g xs) = fold (\acc x. f acc (g x)) z xs
\end{lstlisting}
%
However, this rule cannot be used for a parallel fold. In order to be executed in parallel, the first argument to a fold must be associative. This is not necessarily the case in the right hand side of this rule, not without knowing more about @g@. In fact, the two arguments to the function may not even be of the same type.

Because of these drawbacks, a more general approach is required. One that does more than just simple rewrites of the syntax tree.
...
\end{lstlisting}
%
where @:.@ is the \emph{snoc} constructor defined as
%
\begin{lstlisting}
data sh :. i = sh :. i
\end{lstlisting}
%
It allows us to build lists where the number (and type) of elements is tracked in the type. For example
%
\begin{lstlisting}
Z:.1:.2:.3 :: Z:.Int:.Int:.Int
\end{lstlisting}
%
It is called the snoc constructor as it is used for lists where the rightmost element is outermost in the structure, as opposed to the traditional cons lists in Haskell where the leftmost element is outermost. The @Z@ type is just an alternative version of the unit type.
%
\begin{lstlisting}
data Z = Z
\end{lstlisting}
%
An array of shape @Z@ is called a scalar array and only has 1 element. This was the result type of @dotp@, via this synonym:
%
\begin{lstlisting}
type Scalar = Array Z
\end{lstlisting}
%
Higher dimension arrays can then be built up with @:.@. Accelerate defines some synonyms for common dimensions:
%
\begin{lstlisting}
type DIM0 = Z
type DIM1 = DIM0:.Int
...
\end{lstlisting}
%
From this it should be clear that @Vector@ is just a 1D array.
%
\begin{lstlisting}
type Vector = Array DIM1
\end{lstlisting}
%
When used to describe the dimensionality of an array, we say that the \emph{shape} is the type of the snoc-list and the \emph{extent} a value of that type. So for a $3\times4$ 2D array, its shape would be:
%
\begin{lstlisting}
Z:.Int:.Int
\end{lstlisting}
%
and its extent would be
%
\begin{lstlisting}
Z:.3:.4
\end{lstlisting}
...
@map@ and @zipWith@ behave in much the same way as those same operations over lists. For multidimensional arrays, @zipWith@ yields an array of the minimum extent of all dimensions in the array. For example, if array @a@ has extent
%
\begin{lstlisting}
Z :. 5 :. 4
\end{lstlisting}
...
\end{lstlisting}
%
It constructs an array of the given extent using the supplied function to fill its elements. For example:
%
\begin{lstlisting}
evens :: Exp Int -> Acc (Vector Int)
evens sz = generate (index1 sz) (\ix -> unindex 1ix * 2)
\end{lstlisting}
%
...
%
\item Code generation -- Generates LLVM cide from the AST and passes it off to the LLVM compiler. This stage is different for each backend. The CPU backend generates x86 binaries (with vector instructions) and the PTX backend generates PTX code for CUDA capable GPUs.
%
\item Execution -- Schedules and executes the computations on the target hardware.
\end{itemize}
%

The first one of these stages we won't explore in detail, as it is not a concern of this work. It is sufficient to understand it as the process that produces the first-order syntax in Listing~\ref{lst:acc-ast} from the programmer-friendly higher-order syntax. See \citet{McDonell:acc-optim} for details on how this stage of the pipeline works.

The fusion and optimisation stage will be discussed in Chapter~\ref{chap:implementation} where we will build upon the previous work described in \citet{McDonell:acc-optim}.
...
Execution will be covered in Chapter~\ref{chap:implementation} where we build upon previous work\cite{McDonell:acc-optim,McDonell:2015:acc-llvm,Chakravarty:acc-cuda}.

\subsubsection{Abstract Syntax Tree}

The abstract syntax tree for the core Accelerate language is split into two parts, much like the higher-order syntax exposed to the programmer. Listing~\ref{lst:acc-ast} contains a the data type for @Acc@ stratum, the syntax nodes representing array operations, and Listing~\ref{lst:exp-ast} contains the data type for @Exp@, the nodes for simple scalar operations. Given the number of features that Accelerate offers, there are many different AST nodes. We won't describe all of them here, just describe the basic structure and enough information to form a basis for the remaining chapters.

A key to understanding the definition of the ASTs is know that they are designed to be \emph{type-preserving}, and extensible. Looking at the definition for @Acc@ we have:
%
\begin{lstlisting}
data PreOpenAcc acc aenv a where
\end{lstlisting}
%
From right to left, the type parameters are: the type of term, the type of all free variables in the term (assuming it is an open term), and a recursive continuation. We will describe how these are used to construct an AST, but firstly some background.

The AST uses DeBruijn indices~\cite{DeBruijn1972} to represent variables. This is a common technique when representing ASTs to avoid issues of alpha-equivalence and fresh name generation. Briefly, they work by using the natural numbers in place of variables. The number indicates how many levels of binding outside the super-term the variable is bound. For example
%
\begin{lstlisting}
let A in
  let B in
...
\end{lstlisting}
%
Here, we're calculating the average of @A@ and @B@, where @A@ and @B@ are normal terms. The let-bindings do not introduce a named variable, but the bound values can be referred to with numbers. @v_0@ refers to B as it is the closest binding going up the tree, and @v_1@ refers to @A@ as it is the next binding up.

The typed DeBruijn indices Accelerate uses are defined as
%
\begin{lstlisting}
...
\end{lstlisting}
%
The @env@ parameter is a a left-nested tuple (equivalent to a snoc-list; see \ref{sec:background-accelerate}). The AST node for a variable is then:
%
\begin{lstlisting}
  Avar :: Arrays a
       => Idx aenv a
...
\end{lstlisting}
%
New variables are introduced to environment via let-binding:
%
\begin{lstlisting}
  Alet :: Arrays a
       => acc aenv a
...
These AST nodes also show how the first parameter, the recursive continuation, is used. Rather than simply making the GADT recursive, having an explicit type argument ,used whenever recursion is expected, allows additional information and structure to be embedded in the AST. The original recursion can be recovered with @OpenAcc@:
%
\begin{lstlisting}
newtype OpenAcc aenv a = OpenAcc (PreOpenAcc OpenAcc aenv a)
\end{lstlisting}
%
But also, for example, an AST can be annotated with labels for user-debugging,
%
\begin{lstlisting}
