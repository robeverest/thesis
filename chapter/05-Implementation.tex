...
Now that we have a foundation for regularity aware flattening of nested data parallel programs we will explore what is required to use our program transform in an already implemented, expressive, powerful, high-performance array language and compiler. The implementation we are about to describe is publicly available for inspection and analysis so the description we give will be a high level overview. If more specific detail is required, we refer you to said implementation.

In this chapter, we will first revisit Accelerate's compilation pipeline (from Chapter~\ref{chap:background}) in order to see how our sequences extension fits into that. Then we will explore the implementation of the flattening transform itself. Following this, we will describe how we extended Accelerate's optimisation passes in order to achieve competitive absolute performance of our sequence programs. Finally, we will talk about the dynamic component of the scheduling necessary to execute sequence computations.
